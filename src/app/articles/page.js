...
const HomePage = ({ params }) => {
  ...
  useEffect(() => {
    if (decodedCategoryName) {
      fetch(`https://api.theeverestnews.com/api/articles/all`)
        .then(response => response.json())
        .then(data => {
          const sortedArticles = data.data.sort((a, b) => new Date(b.updatedAt) - new Date(a.updatedAt));
      ...
      <div className="mx-3 md:mx-10 lg:mx-20">
        <ul className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2">
          {displayedArticles.map((article, index) => (
            <li key={article._id} className={`bg-white rounded-lg shadow-md overflow-hidden ${index === 0 ? 'lg:col-span-2 lg:row-span-2' : ''}`}>
                ...
                <a className="block cursor-pointer" onClick={() => handleArticleClick(article._id)}>
                  {article.photos && article.photos.length > 0 && (
                    <img
                      ...
                      className={`w-full ${index === 0 ? 'h-100' : 'h-40'} object-cover`}
                    />
                  )}
                  <div className="p-4">
                    <h2 className={`text-${index === 0 ? '2xl' : 'lg'} font-semibold text-gray-800 hover:text-blue-600`}>
                      {article.headline}
                    </h2>
                  </div>
                </a>
