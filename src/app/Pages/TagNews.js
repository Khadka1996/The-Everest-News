...
const TagNews = () => {
  ...
  const fetchTagDetails = async () => {
    try {
      ...
      const articlesResponse = await axios.get(`${API_URL}/api/articles/articles/all`);
  ...
  const fetchAuthorDetails = async (authorId) => {
    try {
      const authorResponse = await axios.get(`${API_URL}/api/authors/${authorId}`);
          ...
          {authorDetailsArray[index] && authorDetailsArray[index].photo && (
            <div className="mt-4 flex items-center">
              <Link to={`/author/${article.selectedAuthors[0]}`} className="flex items-center mr-4">
                <img
                  src={`${API_URL}/uploads/authors/${authorDetailsArray[index].photo}`}
                  alt="Author"
