...
const ARTICLES_API = `${API_URL}/api/articles/category/विचार/status/published`;
...
const ArticleCard = React.memo(({ article, onArticleClick, onAuthorClick }) => (
  <div className='bg-white shadow-lg rounded-lg overflow-hidden' style={{ contentVisibility: 'auto' }}>
...
const Bichar = () => {
    ...
    const fetchArticlesAndAuthors = async () => {
      try {
        ...
        const articlesWithAuthorInfo = articles.map((article) => {
          ...
          const articlePhoto = article.photos?.length > 0
  ...
  return (
    <div className='mx-3 md:mx-9 lg:mx-18'>
      ...
      <AdvertisementComponent position='nepali_abovethoughts' />
      <div className='my-2'>
      ...
      <AdvertisementComponent position='nepali_belowthoughts' />
    </div>
  );
};
