...
const TourismGhumfir = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        const [articlesResponse, authorsResponse] = await Promise.all([
          axios.get(`${API_URL}/api/articles/category/घुमफिर/status/published`, { params: { page: DEFAULT_PAGE, limit: ARTICLES_LIMIT } }),
        ...
        console.error('Error fetching tourism articles:', error.response?.data || error.message);
  ...
  return (
    <>    
      <AdvertisementComponent position="nepali_belowghumfir" />

      <div className="mx-3 md:mx-10 lg:mx-20 mt-5">
        {error && <p className="text-red-500 text-center mb-4">{error}</p>}
        {(articles.length > 0 || loading) && (
          <h1 className="text-3xl font-semibold mb-4 text-[#7BB660]">घुमफिर</h1>
        )}
